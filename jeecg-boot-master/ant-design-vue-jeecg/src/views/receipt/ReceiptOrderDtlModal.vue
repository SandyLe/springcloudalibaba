    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-row>
          ...
          <a-col :span="12">
            <a-form-item label="收款时间" :labelCol="labelCol" :wrapperCol="wrapperCol">
...
<script>
  ...
  import {addReceiptOrderDtl,editReceiptOrderDtl,getExpenseList, getPayModeTypeList } from '@/api/api'
  export default {
    name: "ReceiptOrderDtlModal",
    props: {
      sourceBillOrder: {
        type: Object,
        default: () => {}
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if(!this.model.id){
              obj=addReceiptOrderDtl(formData);
            }else{
              obj=editReceiptOrderDtl(formData);
            }
            obj.then((res)=>{
              if(res.success){
