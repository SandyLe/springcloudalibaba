    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="费用">
          <a-input placeholder="请输入费用" v-decorator="[ 'amount', {}]" />
        </a-form-item>
      </a-form>
    </a-spin>
  </a-modal>
...
<script>
  ...
  import {addSaleOrderCost,editSaleOrderCost,getExpenseList,getServiceInstitutionList,duplicateCheck } from '@/api/api'

  export default {
    name: "SaleOrderCostModal",
    props: {
      saleOrder: {
        type: Object,
        default: () => {}
    ...
    methods: {
      ...
      edit (record) {
        ...
        getServiceInstitutionList().then((res) => {
          if (res.success) {
            that.institutionList = res.result;
          }
        })
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'expenseId', 'amount', 'payeeId'))
        });
      ...
      handleOk () {
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            if(!this.model.id){
              obj=addSaleOrderCost(formData);
            }else{
              obj=editSaleOrderCost(formData);
            }
            obj.then((res)=>{
              if(res.success){
