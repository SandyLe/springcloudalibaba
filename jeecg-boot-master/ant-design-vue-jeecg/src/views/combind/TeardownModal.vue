    ...
    <a-card :bordered="false">
        <a-form :form="form">
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="拆卸单号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="备注" :labelCol="hlabelCol" :wrapperCol="wrapperCol">
                  ...
                  <a-card>
                      <form :autoFormCreate="(form) => this.form = form">
                          <a-table :columns="columns" :dataSource="tabledata" :pagination="false" rowKey="id" ref="mtltable">
                              <template v-for="(col, i) in ['mtlId','auxiliaryId', 'unitId','quantity', 'price', 'content', 'action']" :slot="col" clearable slot-scope="text, record, index">
    ...
    methods: {
        ...
        add() {
            if(this.$route.params.id){
                getTeardownOne({"id":this.$route.params.id}).then((res)=>{
                    if (res.result) {
        ...
        edit(record) {
            ...
            this.$nextTick(() => {
                this.form.setFieldsValue(
                    pick(this.model, 'id', 'code', 'mtlId', 'content', 'warehouseId', 'quantity','auxiliaryId', 'unitId')
                )
              ...
              if (!err) {
                  ...
                  httpAction(httpurl, formData, method)
                      ...
                      .finally(() => {
                          ...
                          that.$router.push({ path:'/combind/TeardownList' });
                      })
              }
          })
        },
        ...
        popupCallback(row) {
            this.form.setFieldsValue(
                pick(row, 'code', 'vendorId', 'content', 'warehouseId', 'quantity', 'unitId')
            )
            this.form.setFieldsValue({billDate: this.model.billDate ? moment(this.model.billDate) : null})
        },
        newMember() {
        ...
        remove(key,id) {
            ...
            this.tabledata = newData;
            if(id){
                teardownDtlDelete({"id" : id}).then((res) => {
                    if (res.success) {
                    }
                });
            }
        ...
        backToList() {
            ...
            this.$router.replace({ path:'/combind/TeardownList' });
        },
